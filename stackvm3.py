...
class StackVM3( object ):
   ...
   def iterrun( self, CS, env=None ):
      ...
      while True:
         try:
            while True:
               self.opcode,self.operand = self.CS[self.CP]
               yield self
               self.opcode()
               self.CP += 1
         except INTERRUPT as IRQ:
   ...
   def _trace_instruction( self, hexintegers=False ):
      opCodeStr   = self.ropcodes[ self.opcode ]
      ...
      result = '{0:10s}  {1:10s}'.format(opCodeStr,operandStr)
      return result
   ...
   def op_PUSH( self ):
      ...
      #STACK
      '''
      operand = self.CS[self.CP+1]
      self.SS.append( operand ) # operand = value
      self.CP += 2
   ...
   def op_POPn( self ):                     # POPn <count>
      ...
      #STACK
      '''
      count = self.CS[self.CP+1]
      del self.SS[ 0 - count : ]  # operand = count
      self.CP += 2
   ...
   def op_TOPSET( self ):                   # TOPSET <val>
      ...
      #STACK
      '''
      self.SS[ -1 ] = self.CS[self.CP + 1] # operand = value
      self.CP += 2
   ...
   def op_PUSH_NTH( self ):
      ...
      '''
      operand = self.CS[self.CP + 1]
      self.SS.append( self.SS[ 0 - operand ] ) # operand = offset
      self.CP += 2
   ...
   def op_BIND( self ):
      ...
      '''
      value = self.SS.pop( )
      self.ENV.declLocal( value, self.SS[-1] )
      self.CP += 1
   ...
   def op_BINDG( self ):
      ...
      '''
      symbol = self.SS.pop( )
      self.ENV.declGlobal( symbol, self.SS[-1] )
      self.CP += 1
   ...
   def op_REBIND( self ):
      ...
      '''
      symbol = self.SS.pop( )
      self.ENV.rebind( symbol, self.SS[-1] )
      self.CP += 1
   ...
   def op_DEREF( self ):
      ...
      #ENVIRONMENT
      '''
      self.SS[-1] = self.ENV.get( self.SS[-1] )
      self.CP += 1
   ...
   def op_PUSH_BPOFF( self ):
      ...
      '''
      operand = self.CS[self.CP + 1]
      self.SS.append( self.SS[self.BP + operand] ) # operand = offset
      return self.CP + 2
   ...
   def op_CALLp( self ):
      ...
      try:
         pyFn( self, self.operand ) # operand = numArgs
      except:
   ...
   def op_JMP( self ):
      ...
      #BRANCH
      '''
      self.CP = self.operand # operand = address
   ...
   def op_JMP_EQ( self ):
      '''JMP_EQ <address>
      SS:  [ ..., <value> ]  ->  [ ... ]
      :    Set the CP to <address>, if <value> == 0.
      #BRANCH
      '''
      if self.SS.pop() == 0:
         self.CP = self.operand # operand = address
   ...
   def op_iCMP( self):
      '''iCMP
      SS:  [ ..., <value2>, <value1> ]  ->  [ ..., (<value1> - <value2>) ]
      :    Binary-op; pop 2 args; compute integer comparison; push result.
      #ALU.INTEGER
      '''
      #self.BINARY_OP( op.sub, int, int )
      arg2 = self.SS.pop( )
      #arg1 = self.SS[ -1 ]
      self.SS[ -1 ] = int(self.SS[-1]) - int(arg2)
      #self.CP += 1
      ...
      for label,refList in self.staticRefs.items():
         try:
            ...
            for refAddr,refArg in refList:
               exe[refAddr] = ( exe[refAddr][0], address-1 )
         except:
            pass
